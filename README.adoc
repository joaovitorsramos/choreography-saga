...
he code generation is done during the integration-test phase in the custom “java-client” profile, so that it can be executed by invoking Maven’s verify phase (invoking just integration-test will not work as we will also need the pre- and post- phases).
